...
* Quantitation columns/assays

The =quantCols= argument defaults to all possible quantitative columns
and the user choses one (default would be the first) among these. This
can be implemented with =match.arg()=.

The user could even choose what =quantCol= to use, selecting the
=quantCols= among those that are available in the table.

The SE could even contain all available quantitative assays, setting
the one they chose in first position.
