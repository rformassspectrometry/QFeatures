...
[`data`](https://rdrr.io/r/utils/data.html)`(``"hlpsms"``)`` ``quantCols`` ``<-`` `[`grep`](https://rdrr.io/pkg/BiocGenerics/man/grep.html)`(``"^X"``, `[`colnames`](https://rdrr.io/pkg/BiocGenerics/man/row_colnames.html)`(``hlpsms``)``)`` ``(``qfSingle`` ``<-`` `[`readQFeatures`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md)`(``hlpsms``, quantCols ``=`` ``quantCols``)``)`` ``#> Checking arguments.`` ``#> Loading data as a 'SummarizedExperiment' object.`` ``#> Formatting sample annotations (colData).`` ``#> Formatting data as a 'QFeatures' object.`` ``#> An instance of class QFeatures (type: bulk) with 1 set:`` ``#> `` ``#> [1] quants: SummarizedExperiment with 3010 rows and 10 columns`
...
`(``qfSingle`` ``<-`` `[`readQFeatures`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md)`(``hlpsms``, quantCols ``=`` ``quantCols``, name ``=`` ``"psms"``)``)`` ``#> Checking arguments.`` ``#> Loading data as a 'SummarizedExperiment' object.`` ``#> Formatting sample annotations (colData).`` ``#> Formatting data as a 'QFeatures' object.`` ``#> An instance of class QFeatures (type: bulk) with 1 set:`` ``#> `` ``#> [1] psms: SummarizedExperiment with 3010 rows and 10 columns`
...
`hlpsms``$``FileName`` ``<-`` `[`rep`](https://rdrr.io/r/base/rep.html)`(`` `` `[`rep`](https://rdrr.io/r/base/rep.html)`(`[`paste0`](https://rdrr.io/r/base/paste.html)`(``"run"``, ``1``:``3``, ``".raw"``)``, each ``=`` ``4``)``,`` `` length.out ``=`` `[`nrow`](https://rdrr.io/pkg/BiocGenerics/man/nrow.html)`(``hlpsms``)`` ``)`
...
`(``qfMulti`` ``<-`` `[`readQFeatures`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md)`(``hlpsms``, quantCols ``=`` ``quantCols``, runCol ``=`` ``"FileName"``)``)`` ``#> Checking arguments.`` ``#> Loading data as a 'SummarizedExperiment' object.`` ``#> Splitting data in runs.`` ``#> | | | 0% | |======================= | 33% | |=============================================== | 67% | |======================================================================| 100%`` ``#> Formatting sample annotations (colData).`` ``#> Formatting data as a 'QFeatures' object.`` ``#> An instance of class QFeatures (type: bulk) with 3 sets:`` ``#> `` ``#> [1] run1.raw: SummarizedExperiment with 1004 rows and 10 columns `` ``#> [2] run2.raw: SummarizedExperiment with 1004 rows and 10 columns `` ``#> [3] run3.raw: SummarizedExperiment with 1002 rows and 10 columns`
...
`(``coldata`` ``<-`` `[`DataFrame`](https://rdrr.io/pkg/S4Vectors/man/DataFrame-class.html)`(`` `` quantCols ``=`` ``quantCols``,`` `` condition ``=`` `[`rep`](https://rdrr.io/r/base/rep.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"A"``, ``"B"``)``, ``5``)``,`` `` batch ``=`` `[`rep`](https://rdrr.io/r/base/rep.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"batch1"``, ``"batch2"``)``, each ``=`` ``5``)`` ``)``)`` ``#> DataFrame with 10 rows and 3 columns`` ``#> quantCols condition batch`` ``#> <integer> <character> <character>`` ``#> 1 1 A batch1`` ``#> 2 2 B batch1`` ``#> 3 3 A batch1`` ``#> 4 4 B batch1`` ``#> 5 5 A batch1`` ``#> 6 6 B batch2`` ``#> 7 7 A batch2`` ``#> 8 8 B batch2`` ``#> 9 9 A batch2`` ``#> 10 10 B batch2`

We can now provide the table to
[`readQFeatures()`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md):

`(``qfSingle`` ``<-`` `[`readQFeatures`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md)`(``hlpsms``, quantCols ``=`` ``quantCols``, colData ``=`` ``coldata``)``)`` ``#> Checking arguments.`` ``#> Loading data as a 'SummarizedExperiment' object.`` ``#> Formatting sample annotations (colData).`` ``#> Formatting data as a 'QFeatures' object.`` ``#> An instance of class QFeatures (type: bulk) with 1 set:`` ``#> `` ``#> [1] quants: SummarizedExperiment with 3010 rows and 10 columns`

For convenience, the `quantCols` argument can be omitted when providing
`colData` (`quantCols` are then fetched from this table):

`(``qfSingle`` ``<-`` `[`readQFeatures`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md)`(``hlpsms``, colData ``=`` ``coldata``)``)`` ``#> Checking arguments.`` ``#> Loading data as a 'SummarizedExperiment' object.`` ``#> Formatting sample annotations (colData).`` ``#> Formatting data as a 'QFeatures' object.`` ``#> An instance of class QFeatures (type: bulk) with 1 set:`` ``#> `` ``#> [1] quants: SummarizedExperiment with 3010 rows and 10 columns`

The annotations are retrieved as follows:

`colData``(``qfSingle``)`` ``#> DataFrame with 10 rows and 3 columns`` ``#> quantCols condition batch`` ``#> <integer> <character> <character>`` ``#> X126 NA NA NA`` ``#> X127C NA NA NA`` ``#> X127N NA NA NA`` ``#> X128C NA NA NA`` ``#> X128N NA NA NA`` ``#> X129C NA NA NA`` ``#> X129N NA NA NA`` ``#> X130C NA NA NA`` ``#> X130N NA NA NA`` ``#> X131 NA NA NA`
...
`coldataMulti`` ``<-`` `[`DataFrame`](https://rdrr.io/pkg/S4Vectors/man/DataFrame-class.html)`(``)`` ``for`` ``(``run`` ``in`` `[`paste0`](https://rdrr.io/r/base/paste.html)`(``"run"``, ``1``:``3``, ``".raw"``)``)`` ``{`` `` ``coldataMulti`` ``<-`` `[`rbind`](https://rdrr.io/pkg/BiocGenerics/man/cbind.html)`(``coldataMulti``, `[`DataFrame`](https://rdrr.io/pkg/S4Vectors/man/DataFrame-class.html)`(``runCol ``=`` ``run``, ``coldata``)``)`` ``}`` ``coldataMulti`` ``#> DataFrame with 30 rows and 4 columns`` ``#> runCol quantCols condition batch`` ``#> <character> <integer> <character> <character>`` ``#> 1 run1.raw 1 A batch1`` ``#> 2 run1.raw 2 B batch1`` ``#> 3 run1.raw 3 A batch1`` ``#> 4 run1.raw 4 B batch1`` ``#> 5 run1.raw 5 A batch1`` ``#> ... ... ... ... ...`` ``#> 26 run3.raw 6 B batch2`` ``#> 27 run3.raw 7 A batch2`` ``#> 28 run3.raw 8 B batch2`` ``#> 29 run3.raw 9 A batch2`` ``#> 30 run3.raw 10 B batch2`

We can provide the table to
[`readQFeatures()`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md):

`(``qfMulti`` ``<-`` `[`readQFeatures`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md)`(`` `` ``hlpsms``, quantCols ``=`` ``quantCols``, colData ``=`` ``coldataMulti``,`` `` runCol ``=`` ``"FileName"`` ``)``)`` ``#> Checking arguments.`` ``#> Loading data as a 'SummarizedExperiment' object.`` ``#> Splitting data in runs.`` ``#> | | | 0% | |======================= | 33% | |=============================================== | 67% | |======================================================================| 100%`` ``#> Formatting sample annotations (colData).`` ``#> Formatting data as a 'QFeatures' object.`` ``#> An instance of class QFeatures (type: bulk) with 3 sets:`` ``#> `` ``#> [1] run1.raw: SummarizedExperiment with 1004 rows and 10 columns `` ``#> [2] run2.raw: SummarizedExperiment with 1004 rows and 10 columns `` ``#> [3] run3.raw: SummarizedExperiment with 1002 rows and 10 columns`
...
[`colnames`](https://rdrr.io/pkg/BiocGenerics/man/row_colnames.html)`(``qfSingle``)`` ``#> CharacterList of length 1`` ``#> [["quants"]] X126 X127C X127N X128C X128N X129C X129N X130C X130N X131`

In the multi-set case, sample names are the concatenation of the run
name and the quantCols (separated by a `_`).

[`colnames`](https://rdrr.io/pkg/BiocGenerics/man/row_colnames.html)`(``qfMulti``)`` ``#> CharacterList of length 3`` ``#> [["run1.raw"]] run1.raw_X126 run1.raw_X127C ... run1.raw_X130N run1.raw_X131`` ``#> [["run2.raw"]] run2.raw_X126 run2.raw_X127C ... run2.raw_X130N run2.raw_X131`` ``#> [["run3.raw"]] run3.raw_X126 run3.raw_X127C ... run3.raw_X130N run3.raw_X131`
...
`hlpsms``$``X126`` ``<-`` ``NA`` ``(``qfNoEmptyCol`` ``<-`` `[`readQFeatures`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md)`(`` `` ``hlpsms``, quantCols ``=`` ``quantCols``, removeEmptyCols ``=`` ``TRUE`` ``)``)`` ``#> Checking arguments.`` ``#> Loading data as a 'SummarizedExperiment' object.`` ``#> Formatting sample annotations (colData).`` ``#> Formatting data as a 'QFeatures' object.`` ``#> An instance of class QFeatures (type: bulk) with 1 set:`` ``#> `` ``#> [1] quants: SummarizedExperiment with 3010 rows and 9 columns`
...
`(``qfSingle`` ``<-`` `[`readQFeatures`](https://rformassspectrometry.github.io/QFeatures/reference/readQFeatures.md)`(`` `` ``hlpsms``, quantCols ``=`` ``quantCols``, verbose ``=`` ``FALSE`` ``)``)`` ``#> An instance of class QFeatures (type: bulk) with 1 set:`` ``#> `` ``#> [1] quants: SummarizedExperiment with 3010 rows and 10 columns`
...
## Session information
    ...
    loaded via a namespace (and not attached):
     [1] xfun_0.60               bslib_0.12.0            htmlwidgets_1.6.4      
     [4] lattice_0.23-1          crosstalk_1.2.2         vctrs_0.7.3            
     [7] tools_4.6.1             tibble_3.3.1            cluster_2.1.8.3        
    [10] pkgconfig_2.0.3         Matrix_1.7-6            desc_1.4.3             
    [13] lifecycle_1.0.5         compiler_4.6.1          stringr_1.6.0          
    [16] textshaping_1.0.5       clue_0.3-68             htmltools_0.5.9        
    [19] sass_0.4.10             yaml_2.3.12             lazyeval_0.2.3         
    [22] pkgdown_2.2.1.9000      pillar_1.11.1           jquerylib_0.1.4        
    [25] tidyr_1.3.2             MASS_7.3-66             DelayedArray_0.39.6    
    [28] cachem_1.1.0            abind_1.4-8             tidyselect_1.2.1       
    [31] digest_0.6.39           stringi_1.8.9           purrr_1.2.2            
    [34] dplyr_1.2.1             reshape2_1.4.5          bookdown_0.48          
    [37] fastmap_1.2.0           grid_4.6.1              cli_3.6.6              
    [40] SparseArray_1.13.2      magrittr_2.0.5          S4Arrays_1.13.0        
    [43] rmarkdown_2.32          XVector_0.53.0          igraph_2.3.3           
    [46] otel_0.2.0              ragg_1.5.2              evaluate_1.0.5         
    [49] knitr_1.51              rlang_1.3.0             Rcpp_1.1.2             
    [52] glue_1.8.1              BiocManager_1.30.27     jsonlite_2.0.0         
    [55] AnnotationFilter_1.37.0 R6_2.6.1                plyr_1.8.9             
    [58] systemfonts_1.3.2       fs_2.1.0                ProtGenerics_1.45.0    
    [61] MsCoreUtils_1.25.4     
