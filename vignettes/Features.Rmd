...
[@Ramos:2017].  that stores a set of assays. Assays in a `Features`
object have a specific relation, that is depicted in figure
\@ref(fig:featuresplot): assays in a `Features` object are the result
...
```{r featuresplot, fig.cap = "Conceptual representation of a `Features` object and the aggregative relation between different assays.", echo = FALSE}
par(mar = c(0, 0, 0, 0))
...
characteristic of `Features` objects is the aggregative relation
between their assays. This can be obtained with the `aggregateFeatures`
function that will aggregate quantitative features from
one assay into a new one. In the next code chunk, we aggregate PSM-level
data into peptide by grouping all PSMs that were matched the same
peptide sequence. The default aggregation function is median, and can
be set as an additional argument. The new assay is named *peptides*.

```{r aggregateFeatures1}
hl <- aggregateFeatures(hl, "psms", "Sequence", name = "peptides")
hl
hl[["peptides"]]
```
...
hl <- aggregateFeatures(hl, "peptides", "ProteinGroupAccessions", name = "proteins")
hl
hl[["proteins"]]
```

The sample assayed in a `Features` object can be documented in the
...
One particularity of the `Features` infrastructure is that the
...
`aggregateFeatures` and is exploited when subsetting `Feature` by their
feature names.
...
We can easily visualise this new *Features* object using `ggplot2`
...
namely P42227-2 and P42225. We obtain a new `Features` instance
...
respective assays of a `Features` object.

```{r display, eval = FALSE}
display(stat)
```

```{r heatmapdisplay, results='markup', fig.cap="`Features` interactive interface: heatmap of the peptide assay data.", echo=FALSE, out.width='100%', fig.align='center', fig.wide = TRUE}
knitr::include_graphics("./figs/display_hmap.png")
```

```{r assaydisplay, results='markup', fig.cap="`Features` interactive interface: quantitative peptide assay data.", echo=FALSE, out.width='100%', fig.align='center', fig.wide = TRUE}
knitr::include_graphics("./figs/display_assay.png")
```
```{r rowdatadisplay, results='markup', fig.cap="`Features` interactive interface: peptide assay row data", echo=FALSE, out.width='100%', fig.align='center', fig.wide = TRUE}
knitr::include_graphics("./figs/display_rowdata.png")
```
...
Features is assays can also be filtered based on variables in their
