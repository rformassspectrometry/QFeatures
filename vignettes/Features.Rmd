...
between their assays. This can be obtained with the `combineFeatures`
function that will combine (or aggregate) quantitative features from
one assay into a new one. In the next code chunk, we combine PSM-level
...
`combineFeatures` and is exploited when subsetting `Feature` by their
feature names.
