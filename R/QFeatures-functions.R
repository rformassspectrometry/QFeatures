...
##' @title Create precursor identfier
##'
##' @description The [createPrecursorId()] is used to create new precursor
##'     identifier columns in a `QFeatures` object's assays (more precisely in
##'     their rowData). The new variable is called by default `"Precursor.Id"`,
##'     and is generated by the concatenation of other rowData variables that,
...
##'     `c("Modified.Sequence", "Precursor.Charge")`.
##'
...
##' rowDataNames(feat3)
##'
##' res <- createPrecursorId(feat3,
##'                          name = "NewId",
##'                          fcols = c("pval", "Sequence"))
##' rowDataNames(res)
##' rowData(res[[1]])[, c("Sequence", "pval", "NewId")]
...
duplicatedPrecursorId <- function(x) {
    ans <- rep(TRUE, length(x))
    ans[names(which(table(x) == 1))] <- FALSE
    unname(ans)
}
